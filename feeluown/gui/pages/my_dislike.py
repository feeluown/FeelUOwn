...
async def render(req, **kwargs):
    ...
    if isinstance(provider, SupportsCurrentUserDislikeSongsReader):
        reader = await run_fn(provider.current_user_dislike_create_songs_rd)
        songs = reader.readall()
        # TODO: add a widget to show the songs
        ...
        if mtype is ModelType.song:
            ...
            if isinstance(self._provider, SupportsCurrentUserDislikeRemoveSong):
                self.songs_table.remove_song_func = \
                    self._provider.current_user_dislike_remove_song
        else:
            err = '未知类型资源'
