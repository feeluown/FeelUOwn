...
class SongSuggestionItemWidget(QWidget):
    ...
    def __init__(self, song: BriefSongModel, list_view: QListWidget):
        ...
        self.add_btn.setToolTip(t("ai-chat-link-add-to-playlist"))
...
class AIArtifactSongListView(QListWidget):
    ...
    def contextMenuEvent(self, event):
        ...
        menu = QMenu(self)
        menu.addAction(t("ai-chat-link-search")).triggered.connect(
            lambda: self._show_search(song)
        )
        menu.addAction(t("ai-chat-link-play")).triggered.connect(
            lambda: self._play_display_song(song)
        )
        menu.addAction(t("ai-chat-link-add-to-playlist")).triggered.connect(
            lambda: aio.run_afn(self._add_display_song, song)
        )
...
class AIChatBox(QWidget):
    ...
    async def exec_user_query(self, query: str):
        ...
        if ai_radio is not None:
            ...
            self.input_widget.enable_send(False)
            try:
                result = await ai_radio.update_candidates(query)
            except Exception:
                logger.exception("AI radio candidates update failed")
                message = t("ai-radio-candidates-update-failed")
                self.input_widget.set_msg(message, level="err")
                self.history_widget.add_message("assistant", message)
                self._app.show_msg(message)
            else:
                if result.added_songs:
                    message = t(
                        "ai-radio-candidates-updated",
                        count=len(result.added_songs),
                    )
                elif result.removed_count:
                    message = t("ai-radio-candidates-cleared")
                else:
                    message = t("ai-radio-candidates-kept")
                self.history_widget.add_message("assistant", message)
                self.input_widget.set_msg(t("ai-radio-chat-hint"))
                self._app.show_msg(message)
            finally:
                self.input_widget.enable_send(True)
            return
    ...
    def _get_active_ai_radio(self):
        ai_radio = self._app.ai.radio
        if ai_radio is not None and ai_radio.is_active:
            return ai_radio
        return None
...
class Body(QWidget):
    def __init__(self, app: "GuiApp"):
        ...
        self._collapse_btn = TextButton(t("ai-chat-collapse"), height=26)
    ...
    def _schedule_palette_refresh(self, force=False):
        ...
        QTimer.singleShot(50, self._apply_palette)
        QTimer.singleShot(120, self._apply_palette)
        QTimer.singleShot(350, self._apply_palette)
    ...
    def _on_link_context_menu_requested(self, url: str, global_pos):
        ...
        menu = QMenu(self)
        menu.addAction(t("ai-chat-link-search")).triggered.connect(
            lambda: self._show_link_search(model)
        )
        menu.addAction(t("ai-chat-link-play")).triggered.connect(
            lambda: aio.run_afn(self._play_link_model, model)
        )
        menu.addAction(t("ai-chat-link-add-to-playlist")).triggered.connect(
            lambda: aio.run_afn(self._add_link_model, model)
        )
        menu.addSeparator()
        menu.addAction(t("ai-chat-link-copy")).triggered.connect(
    ...
    def _connect_ai_radio_status(self):
        ...
        if ai_radio is self._connected_ai_radio:
            return
        if (
            self._connected_ai_radio is not None
            and hasattr(self._connected_ai_radio, "status_changed")
        ):
            self._connected_ai_radio.status_changed.disconnect(
                self._on_ai_radio_status_changed
            )
        self._connected_ai_radio = ai_radio
        if ai_radio is not None and hasattr(ai_radio, "status_changed"):
...
class AIChatOverlay(AppOverlayContainer):
    def __init__(self, app: "GuiApp", body: Body, parent=None):
        ...
        self._titlebar_mode = MacOSNativeTitlebarMode(app)
