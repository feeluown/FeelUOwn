...
class SongsTable(MusicTable):
    ...
    download_song_signal = pyqtSignal([SongModel])
    ...
    def add_item(self, song_model):
        ...
        # FIXME
        album_item = QTableWidgetItem(getattr(song_model.album, 'name', 'xxx'))
        artist_item = QTableWidgetItem(song_model.artists_name)
        m, s = parse_ms(song_model.duration)
    ...
    def on_cell_dbclick(self, row, column):
        ...
        if column == 0:
            ...
            logger.debug('Double click to play song: %s', song)
        elif column == 2:
            self.show_artist_signal.emit(song.artists[0].identifier)
        elif column == 3:
            self.show_album_signal.emit(song.album.identifier)
