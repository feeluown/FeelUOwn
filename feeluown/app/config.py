...
def create_config() -> Config:
    ...
    config.deffield(
        "ENABLE_MV_AS_STANDBY", type_=int, default=1, desc="MV 作为备用资源"
    )
    ...
    config.deffield(
        "NOTIFY_DURATION", type_=int, default=3000, desc="桌面通知保留时长(ms)"
    )
    config.deffield(
        "PLAYBACK_CROSSFADE", type_=bool, default=False, desc="播放暂停淡入淡出"
    )
    config.deffield(
        "PLAYBACK_CROSSFADE_DURATION", type_=int, default=500, desc="淡入淡出持续时间"
    )
    config.deffield(
        "OPENAI_API_BASEURL", type_=str, default="", desc="OpenAI API base url"
    )
