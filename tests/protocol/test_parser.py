...
from fuocore.protocol import Lexer, Parser, FuoSyntaxError
...
class TestParser(TestCase):
    ...
    def test_read_cmd_error(self):
        ...
        with self.assertRaises(FuoSyntaxError):
            Parser(lexer.tokenize(text), text).parse()
    ...
    def test_parse_cmd_options_3(self):
        ...
        req = Parser(lexer.tokenize('p []'), 'p []').parse()
    ...
    def test_parse_req_options(self):
        ...
        tokens = lexer.tokenize(source)
        req = Parser(tokens, source).parse()
